...
class GCNLayer(nn.Module):
    ...
    def forward(self, x, adj):
        # 图卷积操作：聚合邻居特征
        return F.relu(torch.matmul(adj, torch.matmul(x, self.weight)))

class Transformer(nn.Module):

    def __init__(self, num_set, num_genes, graph_dict, mask, fe_bias=True,
    ...
    def forward_features(self, x, adj):
        ...
        # 图卷积处理
        x = self.gcn_layer(x, adj)  # 在这里添加图卷积层
    ...
    def forward(self, x, adj):
        latent, attn_weights = self.forward_features(x, adj)
...
def scgaa_model(num_set, num_genes, graph_dict,mask, embed_dim=48,depth=2,num_heads=4,has_logits: bool = True):
