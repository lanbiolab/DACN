...
# def generate_weighted_adj_mat(adata, include_self=False, n=6):
#     import numpy as np
#     from sklearn import metrics
#
#     # Ensure spatial data is present
#     assert 'spatial' in adata.obsm, 'AnnData object must include spatial information in `obsm["spatial"]`.'
#
...
#             if dist[i, j] != 0:  # Avoid division by zero
#                 adj_mat[i, j] = dist[i, j]
#
#
#     if not include_self:
#         np.fill_diagonal(adj_mat, 0)  # Fill diagonal with 0 to remove self-loops
#
#     # Make the graph symmetric
#     adj_mat = np.maximum(adj_mat, adj_mat.T)  # Ensure symmetry
#
#     # Normalize the adjacency matrix (row-wise normalization)
#     row_sums = adj_mat.sum(axis=1)  # Calculate row sums
#     row_sums[row_sums == 0] = 1  # Prevent division by zero
#     adj_mat = adj_mat / row_sums[:, np.newaxis]  # Normalize each row by its sum
#
#     return adj_mat
...
def graph_construction(adata, n=6, dmax=50, mode='KNN'):
    ...
    adj_m1 = adj_m1.tocoo()
    shape = adj_m1.shape
    values = np.ones_like(adj_m1.data)
